...
{  
	...
	{  
		...
		{  
			...
			//端点: 网络地址和服务端口的组合. TCP使用一个网络地址和一个服务端口号来唯一标识设备和服务
		...
		{  
			...
			{  
				...
				Console.Write(clientSockets.Count);
		...
		{  
			...
			{  
				...
				{  
					...
					var e = clientSockets.GetEnumerator();
					while(e.MoveNext())	
					{
						var c = e.Current;
						//向连接的客户端发送连接成功的数据  
//						buff.WriteString("Connected Server");  
						ByteBuffer writer = new ByteBuffer();
						writer.WriteString(data);
						c.Send(WriteMessage(writer.ToBytes()));  
