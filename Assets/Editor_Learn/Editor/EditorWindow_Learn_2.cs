...
/// 
/// 思考:
/// 1.GUILayout 和 EditorGUILayout 的区别 
/// 2.有特定类型的Editor类可以继承, 和自定义EditorWindow的有什么区别 https://docs.unity3d.com/ScriptReference/UI.AnimationTriggersDrawer.html
/// 3.undo, 用脚本指定的操作无法快捷键撤销, 必须手动注册,把当前操作对象压入栈 Undo.RegisterCompleteObjectUndo (undo类可以perform ,redo, record等),也不会常用到啦,完善编辑器插件可能需要  https://docs.unity3d.com/ScriptReference/Undo.html
/// 4. EditorUtility.SetDirty 通知编辑器保存, 但是这里不知道效果怎么表现的
/// 
/// 参考:
/// 
/// 官方API,超多属性,一点就送,要什么样式自己找 https://docs.unity3d.com/ScriptReference/EditorGUILayout.html
/// 
/// 大概跟着做下,其实看下就成 http://blog.csdn.net/jjiss318/article/details/7435708
/// </summary>
public class EditorWindow_Learn_2 : EditorWindow
{
    ...
    /// </summary>
    [MenuItem("Editor Tool/EditorWindow 2")]
    static void Init()
    {
        EditorWindow_Learn_2 window = (EditorWindow_Learn_2)EditorWindow.GetWindow(typeof(EditorWindow_Learn_2));
        window.Show();
    }
        ...
        if (GUILayout.Button("Align"))
        {
            if (source != null)
            {
                ...
                foreach (Transform t in Selection.transforms)
                {
                    ...
                    //Register the Undo
                    Undo.RegisterCompleteObjectUndo(t, "Align " + t.gameObject.name + " to " + source.gameObject.name);
                    t.position = tmp;
        ...
        if (GUILayout.Button("Copy Position"))
        {
            if (source != null)
            {
                foreach (Transform t in Selection.transforms)
                {
                    ...
                    Undo.RegisterCompleteObjectUndo(t, "Copy position");
                    t.position = tmp;
                }
            }
            else
        ...
        if (GUILayout.Button("Copy Rotation"))
        {
            if (source != null)
            {
                foreach (Transform t in Selection.transforms)
                {
                    ...
                    Undo.RegisterCompleteObjectUndo(t, "Copy rotation");
                    t.rotation = tmp2;
                }
            }
            else
        ...
        if (GUILayout.Button("Copy Local Scale"))
        {
            if (source != null)
            {
                foreach (Transform t in Selection.transforms)
                {
                    ...
                    Undo.RegisterCompleteObjectUndo(t, "Copy local scale");
                    t.localScale = tmp;
                }
            }
            else
    ...
    private void CreateAddNoiseToTransformWindow()
    {
        ...
        if (GUILayout.Button("Add noise to Position"))
        {
            foreach (Transform t in Selection.transforms)
            {
                ...
                Undo.RegisterCompleteObjectUndo(t, "Add noise to position");
                t.position += tmp;
            }
        }
        ...
        if (GUILayout.Button("Add noise to Rotation"))
        {
            foreach (Transform t in Selection.transforms)
            {
                ...
                Undo.RegisterCompleteObjectUndo(t, "Add noise to rotation");
        ...
        if (GUILayout.Button("Add noise to Local Scale"))
        {
            foreach (Transform t in Selection.transforms)
            {
                ...
                Undo.RegisterCompleteObjectUndo(t, "Add noise to local scale");
                t.localScale += tmp;
            }
        }
    }
